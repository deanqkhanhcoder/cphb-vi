\documentclass[twoside,12pt,a4paper,openany,vietnamese]{book}
%\documentclass[twoside,12pt,a4paper,vietnamese]{book}

%\includeonly{chapter04,list}

\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage[vietnamese]{babel}
\usepackage{vntex}
\usepackage{listings}
\usepackage[table]{xcolor}
\usepackage{tikz}
\usepackage{multicol}
\usepackage{hyperref}
\usepackage{array}
\usepackage{microtype}

\usepackage{fouriernc}

\usepackage{graphicx}
\usepackage{framed}
\usepackage{amssymb}
\usepackage{amsmath}

\usepackage{pifont}
\usepackage{ifthen}
\usepackage{makeidx}
\usepackage{enumitem}

\usepackage{titlesec}

\usepackage{skak}
\usepackage[scaled=0.95]{inconsolata}


\usetikzlibrary{patterns,snakes}
\pagestyle{plain}

\definecolor{keywords}{HTML}{44548A}
\definecolor{strings}{HTML}{00999A}
\definecolor{comments}{HTML}{990000}

\lstset{language=C++,frame=single,basicstyle=\ttfamily \small,showstringspaces=false,columns=flexible}
\lstset{
  literate={ö}{{\"o}}1
           {ä}{{\"a}}1
           {ü}{{\"u}}1
}
\lstset{xleftmargin=20pt,xrightmargin=5pt}
\lstset{aboveskip=12pt,belowskip=8pt}

\lstset{
    commentstyle=\color{comments},
    keywordstyle=\color{keywords},
    stringstyle=\color{strings}
}

\date{Draft \today}

\usepackage[a4paper,top=20mm,bottom=20mm,left=20mm,right=20mm]{geometry}

\title{\Huge Competitive Programmer's Handbook}
\author{\Large Antti Laaksonen (deanqkhanh dịch)}

\makeindex
\usepackage[totoc]{idxlayout}

\titleformat{\subsubsection}
{\normalfont\large\bfseries\sffamily}{\thesubsection}{1em}{}
\begin{document}

%\selectlanguage{finnish}

%\setcounter{page}{1}
%\pagenumbering{roman}

\frontmatter
\maketitle
\setcounter{tocdepth}{1}
\tableofcontents

\include{preface}

\mainmatter
\pagenumbering{arabic}
\setcounter{page}{1}

\usepackage{xspace}
\newcommand{\code}[1]{\texttt{#1}\xspace}
\newcommand{\F}{\code{first}\xspace}
\newcommand{\S}{\code{second}\xspace}
\newcommand{\PB}{\code{push\_back}\xspace}
\newcommand{\MP}{\code{make\_pair}\xspace}
\newcommand{\REP}[3]{\for (int #1 = #2; #1 <= #3; #1++)}

\newcommand{\key}[1] {\textbf{#1}}

\part{Basic techniques}
\include{chapter01}
\include{chapter02}
\include{chapter03}
\include{chapter04}
\include{chapter05}
\include{chapter06}
\include{chapter07}
\include{chapter08}
\include{chapter09}
\include{chapter10}
\part{Graph algorithms}
\include{chapter11}
\include{chapter12}
\include{chapter13}
\include{chapter14}
\include{chapter15}
\include{chapter16}
\include{chapter17}
\include{chapter18}
\include{chapter19}
\include{chapter20}
\part{Advanced topics}
\include{chapter21}
\include{chapter22}
\include{chapter23}
\include{chapter24}
\include{chapter25}
\include{chapter26}
\include{chapter27}
\include{chapter28}
\include{chapter29}
\include{chapter30}

\clearpage
\phantomsection
\addcontentsline{toc}{chapter}{Tài liệu tham khảo}

\include{list}

\cleardoublepage
\printindex

\end{document}